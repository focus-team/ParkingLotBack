...
public interface ParkingOrderRepository extends JpaRepository<ParkingOrder,Long> {
    ...
    @Query(value =
            "select * from parking_order WHERE parking_boy_id = 0 AND user_id IN (SELECT id FROM USER WHERE tag IN (SELECT tag FROM parking_boy WHERE id = :parkingBoyId))",nativeQuery = true)
